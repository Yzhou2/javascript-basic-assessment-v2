...
for (var prop in briefing) {
  if (prop.substr(0,word.length)===word){
...
function removeThings(things, thing) {
...
// repeats strings. Example: 4->12, 2.5->7.5, 'Awesome'->'AwesomeAwesomeAwesome'
  function tripleTheFun(par) {
    if (typeof par === 'number') {
      return par*3;
    } else if ( typeof par === 'string' && !isNaN(Number(par))) {
      return par*3;
    } else if (typeof par ==='string' && isNaN(Number(par))) {
      return par.repeat(3);
    }
...
// #14 Create a function called spiderTest that takes in an object that is an actor, it looks to see if the actor's name is Andrew Garfield and changes 
//the actor object to have a property called spiderRating with a value of 10.  If their name is Tobey Maguire set spiderRating to 0.  
...
// #15 Create a function called whatIsItOutside that takes in a temperature (number), a humidity(number), and a cloudiness(number), 
...
function whatIsItOutside(temperature,humidity,cloudiness) {
  if(temperature>85 && humidity>45) return "I'm all sweat";
  else if(temperature<40 && cloudiness>60) return "I have icicles";
  else if (temperature>85 && humidity<45 && cloudiness<20)  return "I'm literally in the desert";
  else if (temperature>90 || humidity>55 || cloudiness>65) return "Hmm, probably not";
